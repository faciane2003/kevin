...
export const AlleyFog: React.FC<AlleyFogProps> = ({ scene, enabled, count = 24, radius = 420 }) => {
  useEffect(() => {
    if (!scene || !enabled) return;
    const root = new TransformNode("alley_fog_root", scene);
    const mat = new StandardMaterial("alley_fog_mat", scene);
    mat.diffuseColor = new Color3(0.6, 0.65, 0.75);
    mat.alpha = 0.12;
    mat.backFaceCulling = false;
    const fogs: Mesh[] = [];
    for (let i = 0; i < count; i += 1) {
      const plane = MeshBuilder.CreatePlane(`alley_fog_${i}`, { size: randRange(10, 18) }, scene);
      plane.position = new Vector3(randRange(-radius, radius), randRange(2, 6), randRange(-radius, radius));
      plane.billboardMode = Mesh.BILLBOARDMODE_Y;
      plane.material = mat;
      plane.parent = root;
      plane.metadata = { lodGroup: "alleyFog", lodDistance: 600 };
      fogs.push(plane);
    }
    const start = performance.now();
    const obs = scene.onBeforeRenderObservable.add(() => {
      const t = (performance.now() - start) / 1000;
      fogs.forEach((f, idx) => {
        f.position.y += Math.sin(t * 0.4 + idx) * 0.004;
      });
    });
    return () => {
      scene.onBeforeRenderObservable.remove(obs);
      fogs.forEach((f) => f.dispose());
      mat.dispose();
      root.dispose();
    };
  }, [scene, enabled, count, radius]);
