...
const GroundFog: React.FC<Props> = ({ scene, settings }) => {
  ...
  useEffect(() => {
    if (!scene) return;
    const root = new TransformNode("groundFogRoot", scene);
    rootRef.current = root;

    const gradient = new DynamicTexture("groundFogGradient", { width: 64, height: 256 }, scene, false);
    gradientRef.current = gradient;

    const mat = new StandardMaterial("groundFogMat", scene);
